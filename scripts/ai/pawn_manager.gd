...
var _cached_enemies: Array[Pawn] = []
var _enemy_cache_tick: int = -1

func _should_interrupt_for_combat(p: Pawn) -> bool:
	if _enemy_cache_tick != current_tick_cached:
		_cached_enemies.clear()
		for other: Pawn in pawns:
			if other.dead or other.downed:
				continue
			if other.has_meta("faction") and other.get_meta("faction") == "enemy":
				_cached_enemies.append(other)
		_enemy_cache_tick = current_tick_cached
	if _cached_enemies.is_empty():
		return false
	for enemy: Pawn in _cached_enemies:
		if p.grid_pos.distance_to(enemy.grid_pos) <= JobGiverFight.THREAT_RANGE:
			return true
	return false
